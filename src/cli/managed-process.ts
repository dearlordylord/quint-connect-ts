...
const processStartError = (cause: unknown): ProcessStartError => {
  ...
  if (Option.isSome(decoded)) {
    return new ProcessStartError(decoded.value)
  }
  return new ProcessStartError({ message: String(cause) })
}
...
export const runManagedProcess = <Process extends ManagedProcess, Result>(
  ...
  Effect.scoped(
    Effect.gen(function*() {
      ...
      return yield* Effect.callback<Result, ProcessStartError>((resume) => {
